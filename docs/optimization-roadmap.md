...
| --- | --- | --- | --- | --- |
| ALG-01 | Replace heuristic country->zone fallback with curated provider zone mapping table | P0 | IN_PROGRESS | Remove hardcoded generic fallbacks like `US -> US-NY`; mapping is auditable and test-covered |
...
| ID | Item | Priority | Status | Acceptance Criteria |
| --- | --- | --- | --- | --- |
| SCH-01 | Add no-regret wait guard for `run-aware` (max acceptable delay for marginal gain) | P0 | TODO | Command avoids long waits for tiny emission gains; deterministic and configurable |
...
| ID | Item | Priority | Status | Acceptance Criteria |
| --- | --- | --- | --- | --- |
| REL-01 | Standardize provider error taxonomy (auth/rate-limit/network/upstream/invalid-data) | P0 | TODO | Errors map predictably to exit codes and JSON error contract |
...
| ID | Item | Priority | Status | Acceptance Criteria |
| --- | --- | --- | --- | --- |
| UX-01 | Add JSON schema version field for all machine outputs | P0 | TODO | JSON outputs contain stable `schema_version` and changelog policy |
...
| --- | --- | --- |
| M1 | ALG-01, REL-01, UX-01 | Stabilize correctness and output contract |
